 ...
 */
export const revalidateRunningSite = async (): Promise<void> => {
  ...
  try {
    for (const tag of tags) {
      const response = await fetch(`${siteUrl()}/api/revalidate?tag=${tag}`, {
        method: 'POST',
    ...
    console.log('  cache: revalidated the running site')
  } catch (error) {
    console.log(
      `  cache: could not reach ${siteUrl()} (${
